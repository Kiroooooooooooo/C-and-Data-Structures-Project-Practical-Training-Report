...
void View::InsertView()
{
	...
	cout<<"请输入城市名："<<endl; 
	cin >> stu.city;
	cout<<"请输入该城市森林覆盖率（该项重要，请认真填写）："<<endl;
	cin >> stu.area;
...
void View::DelView()
{
	...
	if(index<length){
		...
		if('y'==x){
			...
			cout<<"森林覆盖率为"<<stu.area<<"的城市已被删除！"<<endl; 
		}
	}
	else{
		cout<<"对不起，您输入的森林覆盖率有误，请重新输入"<<endl;
	}
	cout<<"请输入任意值退出"<<endl;
	char n;
	cin >> n;
